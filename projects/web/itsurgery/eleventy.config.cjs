...
module.exports = function (eleventyConfig) {
  ...
  // {hourlyRate}-style placeholders in FAQ text, filled from site.json so the
  // answers can never disagree with the prices shown elsewhere.
  const siteData = require("./src/_data/site.json");
  eleventyConfig.addFilter("fill", (text) =>
    String(text).replace(/\{(\w+)\}/g, (m, k) => {
      if (k === "areas") return siteData.areas.slice(0, -1).join(", ") + " and " + siteData.areas.slice(-1);
      return k in siteData ? siteData[k] : m;
    })
  );
