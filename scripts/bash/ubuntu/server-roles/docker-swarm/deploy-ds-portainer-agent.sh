...
# Prompts once for SSH username and password, reusing them for all SSH and sudo commands.
# Checks/removes existing Portainer Agent services/images, verifies IPTABLES rules, creates an overlay network,
# sets up the storage directory, and deploys the Portainer Agent globally.
# Uses sshpass for non-interactive SSH and sudo -S for remote sudo commands.
# Includes retries, node health checks, IPTABLES validation, and detailed logging.
# Prerequisites: Docker Swarm initialized, storage path accessible, sudo privileges, Docker installed, sshpass installed.
...

{
    ...
    echo "Enter the SSH username (default: pos-admin):"
    read SSH_USERNAME
    SSH_USERNAME=${SSH_USERNAME:-pos-admin}
    ...
    if ! command -v sshpass &> /dev/null; then
        echo "Error: sshpass is not installed. Install it with: sudo apt-get install -y sshpass" | tee -a "$LOG_FILE"
        exit 1
    fi
