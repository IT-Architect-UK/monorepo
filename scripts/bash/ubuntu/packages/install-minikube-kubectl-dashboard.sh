...
if ! command_exists docker; then
    ...
    echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] [invalid url, do not cite] $(lsb_release -cs) stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null
...
# Enable metrics-server addon for full dashboard features
...
# List services in kubernetes-dashboard namespace for logging
log "Listing services in kubernetes-dashboard namespace..."
log_command kubectl get services -n kubernetes-dashboard
...
if ! kubectl get service -n kubernetes-dashboard kubernetes-dashboard > /dev/null 2>&1; then
    log "Service kubernetes-dashboard not found in namespace kubernetes-dashboard."
    exit 1
fi

# Get the dashboard service port
DASHBOARD_PORT=$(kubectl get service -n kubernetes-dashboard kubernetes-dashboard -o jsonpath='{.spec.ports[0].port}')
if [ -z "$DASHBOARD_PORT" ]; then
    log "Failed to get dashboard service port."
    exit 1
fi
log "Dashboard service port: $DASHBOARD_PORT"

# Start kubectl port-forward for dashboard LAN access
log "Starting kubectl port-forward for dashboard LAN access..."
log_command kubectl port-forward --address 0.0.0.0 -n kubernetes-dashboard service/kubernetes-dashboard 8001:$DASHBOARD_PORT &
# Wait for proxy to start
sleep 5

# Test dashboard access locally
log "Testing dashboard access locally..."
if curl -s --insecure "[invalid url, do not cite] | grep -q "Kubernetes Dashboard"; then
