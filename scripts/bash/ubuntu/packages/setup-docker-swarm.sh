...
if [ -z "$BASH_VERSION" ]; then
    ...
    echo "Run it as: bash install_vault.sh or ./install_vault.sh (with proper shebang)"
    exit 1
fi
...
verify_installation() {
    if command -v vault &> /dev/null; then
        ...
        if [ "$CURRENT_VERSION" = "$VAULT_VERSION" ]; then
            ...
            error_exit "Installed version ($CURRENT_VERSION) does not match requested version ($VAULT_VERSION)"
        fi
    else
