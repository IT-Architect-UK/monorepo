...
| Script | Purpose |
|--------|---------|
| `aws/account/aws-account-baseline.sh` | Security baseline for a new account: IAM password policy, account-level S3 public-access block, multi-region CloudTrail to a new S3 bucket, GuardDuty, and deletion of the region's default VPC (skip with `--skip-delete-default-vpc`). Checks for an AWS Config recorder and warns if there is none; it does not create one |
...
`azure/compute/` is an empty placeholder.
