...
#
# This build runs from ANY machine with Packer + Ansible and Proxmox API
# access — three equally supported entry points:
#   1. Semaphore job "Build Golden Image — Ubuntu 24.04" (created by the
#      Deployment Toolbox bootstrap) — zero-touch, scheduled rebuilds
#   2. ./build-ubuntu-2404-proxmox.sh — guided standalone run on Linux
#   3. Raw packer, fully manual:
#        packer init .
#        export PKR_VAR_proxmox_password="..."   # or a token, see below
#        packer build .
#
# Authentication: password (proxmox_password) or API token
# (proxmox_username "user@realm!tokenid" + proxmox_token).
#
...
build {
  ...
  # installed for the baseline run and removed again before sealing — the
  # golden image stays lean.
  provisioner "file" {
