...
command -v xorriso &>/dev/null || fail "xorriso not found — Packer needs it to build the autounattend CD. Install it: sudo apt-get install -y xorriso (baked into toolbox images from the next rebuild)"
