...
command -v xorriso &>/dev/null || fail "xorriso not found — Packer needs it to build the autounattend CD. Install it: sudo apt-get update && sudo apt-get install -y xorriso (baked into toolbox images from the next rebuild)"
...

{
    ...
    # PKR_VAR_win_vm_id was passed — one source of truth, no drift.
    VMID="${PKR_VAR_win_vm_id:-$(grep -A3 'variable "win_vm_id"' "${SCRIPT_DIR}/variables.pkr.hcl" | grep -m1 -oE '[0-9]+')}"
    if [[ -n "${VMID}" ]]; then
